  ...
  return (
    <>
      <div className="flex justify-center items-center min-h-screen bg-neutral-600 ">
        ...
        <div className=" rounded-lg shadow-lg p-6 max-w-3xl w-full bg-neutral-700 m-2">
            ...
            <h2 className='text-white text-xl my-4 flex font-semibold '>Pinned <span className='text-2xl m-1'><VscPinnedDirty /></span></h2>
            {
              pinnedTodos.length > 0 ? pinnedTodos.map((item) => {
                return (
                  ...
                  <div key={item.id} className='todos flex  gap-3  p-2 rounded-xl bg-neutral-700 shadow-md my-2'>
                    <input className='h-7' type='checkbox' checked={item.is_completed ? true : false} id={item.id} onChange={handleCheckboxChange} />
                    <div className="todo">
                      <p className={item.is_completed ? "text-white font-light text-lg line-through" : "text-white font-light text-lg"}>{item.todo}</p>
                    </div>
